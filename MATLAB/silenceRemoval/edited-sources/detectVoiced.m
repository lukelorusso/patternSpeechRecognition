function [outSignal, segments] = detectVoiced(wavSignal, fs, debugMode) % edited by Luca Lorusso
...
% EXECUTION EXAMPLE:
%
% [outSignal] = detectVoiced(example,fs,1);
%


%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Edited by Luca Lorusso
%%%%%%%%%%%%%%%%%%%%%%%%%%%
outSignal = []; % preallocating resulting signal
if fs == 22050 % fixing 22050 sample rate bug
    fs = 22040;
end
if nargin < 3
    debugMode = 0;
...
if (size(wavSignal, 2)==2)
	wavSignal = mean(wavSignal')';
end
...
Eor = ShortTimeEnergy(wavSignal, win*fs, step*fs);
Cor = SpectralCentroid(wavSignal, win*fs, step*fs, fs);
...
while (count < length(flags)) % while there are windows to be processed:
	...
	if (countTemp>1) % if at least one segment has been found in the current loop:
		...
		if (Limit2>length(wavSignal))
            Limit2 = length(wavSignal);
        end
...
for (i=1:size(Limits,1))
    segments{end+1} = wavSignal(Limits(i,1):Limits(i,2)); 
end

% Recompose outSignal - edited by Luca Lorusso
outSignal = cat(1,segments{:});

if debugMode == 1 % plot results:
    subplot(3,1,3);
    % Plot results and play segments:
    time = 0:1/fs:(length(wavSignal)-1) / fs;
    for (i=1:length(segments))
        hold off;
        P1 = plot(time, wavSignal); set(P1, 'Color', [0.7 0.7 0.7]);    
        hold on;
        ...
        axis([0 time(end) min(wavSignal) max(wavSignal)]);
        sound(segments{i}, fs);
        clc;
        fprintf('Playing segment %d of %d. Press any key to continue...\n', i, length(segments)); % edited by Luca Lorusso
        pause
    end
    clc
    hold off;
